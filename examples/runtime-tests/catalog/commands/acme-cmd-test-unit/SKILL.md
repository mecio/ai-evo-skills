---
name: acme-cmd-test-unit
description: Run the project unit test wrapper in its configured PHP context and report the observed exit status.
metadata:
  ai-evo-kind: command
  ai-evo-version: "1.0"
...
Run the project unit test wrapper in its configured PHP context and report the observed exit status.
...
2. Otherwise run `.ai-evo/bin/ai-evo-skills command plan acme-cmd-test-unit` with the current adapter,
...
4. Trim outer whitespace from the supplied `context` locally and require `php83`.
...
5. Run `./scripts/run-unit-tests` from the application Git root. This project-owned executable must
   already select the correct runtime for this checkout, run only the unit suite and propagate the
...
```text
$acme-cmd-test-unit context=php83
```
