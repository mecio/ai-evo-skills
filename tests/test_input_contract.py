...
class InputContractTest(unittest.TestCase):
    ...
    def test_recipe_input_resolver_passes_literal_mapping_before_defaults(self):
        with self.project() as (root, _, path, data):
            resolver = root / '.ai-evo-prj/skills/catalog/commands/abc-report-workflow/SKILL.md'
            resolver.parent.mkdir(parents=True)
            resolver.write_text(fixtures.VALID_COMMAND.replace(
                'name: abc-inspect', 'name: abc-report-workflow').replace(
                'network: disabled', 'network: disabled\n  capabilities: [abc.workflow-report]').replace(
                'inputs: {}', 'inputs:\n  issue: {required: true, description: Issue}\n  requested_recipe: {required: true, description: Recipe}'))
            scripts = root / '.ai-evo-prj/scripts'
            scripts.mkdir()
            helper = scripts / 'workflow-report'
            helper.write_text(
                '#!/usr/bin/env python3\nimport json\nimport sys\n'
                'assert sys.argv == [sys.argv[0], "report", "--issue", "7", "--requested-recipe", "abc-recipe-flow"]\n'
                'print(json.dumps({"recommended_recipe":"abc-recipe-flow","resolved_inputs":{},"missing_inputs":[]}))\n'
            )
            helper.chmod(0o755)
            capabilities = root / '.ai-evo-prj/skills/config/execution-capabilities.yaml'
            capabilities.write_text(yaml.safe_dump({'version': '1.0', 'capabilities': {
            ...
            data['inputs'] = {'issue': {'required': True, 'description': 'Issue'}}
            data['input-resolver'] = {'uses': 'abc-report-workflow', 'with': {
                'issue': '${{ inputs.issue }}', 'requested_recipe': 'abc-recipe-flow',
            }}
            path.write_text(yaml.safe_dump(data))

            planned = self.run_cli(root, 'recipe', 'plan', 'abc-recipe-flow', '--adapter', 'codex', '--input', 'issue=7')
